...
// import { createPaymentIntent } from "../Redux/reducers/slice/paymentSlice";
import { fetchClientSecret } from "../Redux/reducers/slice/paymentSlice";

const stripePromise = loadStripe('pk_test_51O5xx4B2huHs2asNb1CfPHExyxbaf2FMOjYssY2S9WFCeHFnhBKzrw7AaCKQRo8I0zqsi4WHZIlUw9aH7FoOODFH00amsysKgI');
...
const CheckoutForm = () => {
  ...
  useEffect(() => {
    if (cartItems.length > 0) {
      ...
      dispatch(fetchClientSecret(totalAmount));
    }
  }, [dispatch, cartItems]);
  ...
  const handleSubmit = async (event) => {
    ...
    const response = await fetch('/checkout', {
      ...
      body: JSON.stringify({ token: token.id, amount: 5000 }), // amount in cents
    });
    ...
    if (data.error) {
      ...
      // clear cart, navigate to thank you page, etc.
    }
  };
